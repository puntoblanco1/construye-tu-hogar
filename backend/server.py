...
async def add_favorite(property_id: int, request: Request):
    user = await get_current_user(request)
    existing = await db.favorites.find_one({"user_id": user["user_id"], "property_id": property_id}, {"_id": 0})
    if existing:
...
async def remove_favorite(property_id: int, request: Request):
    user = await get_current_user(request)
